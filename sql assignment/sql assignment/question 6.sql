...
SELECT SUM(AMOUNT) "AMOUNT" FROM DEPOSIT WHERE ADATE > 'A-JAN-96';

SELECT SUM(D.AMOUNT) "AMOUNT" FROM CUSTOMERS C INNER JOIN DEPOSIT D ON C.CNAME=D.CNAME WHERE CITY='NAGPUR';

SELECT MAX(D.AMOUNT)"AMOUNT" FROM CUSTOMERS C INNER JOIN DEPOSIT D ON C.CNAME=D.CNAME WHERE CITY='BOMBAY';
...
SELECT COUNT (EMP_SAL) "COUNT" FROM EMPLOYEE WHERE EXTRACT (YEAR FROM HIRE_DATE) = 1995 OR EXTRACT (YEAR FROM HIRE_DATE) = 2000;


SELECT AVG (EMP_SAL) "AVG" , DEPT_NO FROM EMPLOYEE GROUP BY DEPT_NO;


SELECT SUM(EMP_SAL) "SUM" , DEPT_NO FROM EMPLOYEE GROUP BY (DEPT_NO);

SELECT ROUND (AVG(EMP_SAL)) "EMP_SAL" FROM EMPLOYEE WHERE (EMP_SAL>2000) GROUP BY DEPT_NO;

SELECT JOB_NAME , MAX_SAL FROM JOB WHERE MAX_SAL > 3000 ORDER BY (MAX_SAL);

SELECT SUM(AMOUNT) FROM BRANCH B INNER JOIN DEPOSIT D ON B.BNAME=D.BNAME WHERE D.AMOUNT>5000 AND B.CITY='BOMBAY';
