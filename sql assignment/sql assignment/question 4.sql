...
SELECT EMP_NO, EMP_SAL, ROUND (EMP_SAL * 0.15+EMP_SAL) "NEW_SALARY" FROM EMPLOYEE;
SELECT EMP_NO , EMP_SAL, ROUND(EMP_SAL * 0.15+EMP_SAL) "NEW_SALARY" , (EMP_SAL * 0.15+EMP_SAL - EMP_SAL) "NEW_SALARY 1 " FROM EMPLOYEE;
SELECT INITCAP (EMP_NAME)"NAME" , LENGTH(EMP_NAME)"LENGTH" FROM EMPLOYEE WHERE EMP_NAME LIKE 'J%' OR EMP_NAME LIKE 'A%' OR EMP_NAME LIKE 'M%' ORDER BY EMP_NAME;
SELECT (EMP_NAME || 'EARNS' || EMP_SAL || 'MONTHLY') "SALARY" FROM DEPT; 
SELECT EMP_NAME , HIRE_DATE , MONTHS_BETWEEN (SYSDATE , HIRE_DATE) "MONTHS" , EXTRACT (DAY FROM HIRE_DATE) "DAY" FROM EMPLOYEE ORDER BY HIRE_DATE;
SELECT TO_CHAR(HIRE_DATE,'DDSPTH "of" MON YYYY hh:mi:ss AM') FROM EMPLOYEE;
SELECT EMP_NO , EMP_SAL , (EMP_SAL+EMP_COMM) "COMPENSATION" FROM EMPLOYEE;
