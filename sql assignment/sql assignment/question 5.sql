...
SELECT C.CNAME,C.CITY,D.ACTNO,B.AMOUNT,D.ADATE,B.LOANNO,D.AMOUNT FROM CUSTOMERS C INNER JOIN DEPOSIT D ON C.CNAME=D.CNAME INNER JOIN BORROW B ON D.CNAME=B.CNAME WHERE C.CNAME='ANIL';
...
SELECT C.CITY , C.CNAME , B.CNAME FROM CUSTOMERS C INNER JOIN BRANCH B ON C.CITY=B.CITY INNER JOIN DEPOSIT D ON B.BNAME=D.BNAME;

SELECT E.EMP_NAME,D.DNAME,D.DEPT_NO FROM EMPLOYEE E INNER JOIN DEPT D ON E.DEPT_NO=D.DEPT_NO;

SELECT J.JOB_ID,J.JOB_NAME,J.MIN_SAL FROM JOB J  WHERE MIN_SAL<(SELECT EMP_SAL FROM EMPLOYEE WHERE DEPT_NO=30);

SELECT E.EMP_NAME,D.DEPT_NO,D.DNAME FROM EMPLOYEE E INNER JOIN DEPT D ON E.DEPT_NO=D.DEPT_NO  WHERE D.LOCATION='NEW YORK';

SELECT E.EMP_NAME "EMP" , E.EMP_NO , M.MNAME "Manager" , M.MNO "Mgr#" FROM MANAGER M INNER JOIN DEPT D ON M.MNO = D.MNO INNER JOIN EMPLOYEE E ON D.DEPT_NO=E.DEPT_NO;

SELECT EMP_NAME,HIRE_DATE FROM EMPLOYEE WHERE HIRE_DATE > (SELECT HIRE_DATE FROM EMPLOYEE WHERE EMP_NAME='Aman');
