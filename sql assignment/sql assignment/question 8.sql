...
SELECT ACTNO,CNAME,BNAME, (AMOUNT *0.10+AMOUNT) "NEW AMOUNT" FROM DEPOSIT;

SELECT ACTNO,CNAME,BNAME, (AMOUNT * 0.10 +AMOUNT) "NEW AMOUNT" FROM DEPOSIT WHERE BNAME='VRCE';

SELECT D.ACTNO , D.CNAME , D.BNAME , (D.AMOUNT * 0.10 + D.AMOUNT) "NEW AMOUNT" FROM DEPOSIT D INNER JOIN CUSTOMERS C ON C.CNAME = D.CNAME INNER JOIN BRANCH B ON D.BNAME=B.BNAME WHERE B.CITY = 'BOMBAY'  AND C.CITY = 'NAGPUR';

UPDATE  EMPLOYEE SET EMP_NO = 201 WHERE EMP_NO = 105;

SELECT BNAME , (CASE WHEN CNAME IN ('SUNIL') THEN AMOUNT +10 ELSE AMOUNT-10 END) FROM DEPOSIT WHERE BNAME = (SELECT BNAME FROM DEPOSIT WHERE CNAME='ANIL');

SELECT CNAME , (AMOUNT+100)  FROM DEPOSIT WHERE AMOUNT =(SELECT MAX(AMOUNT) FROM DEPOSIT );

DELETE FROM DEPOSIT WHERE BNAME=(SELECT BNAME FROM (SELECT BNAME , COUNT (CNAME) "D" FROM DEPOSIT GROUP BY (BNAME))B WHERE B.D IN (2));

DELETE FROM DEPOSIT WHERE CNAME='ANIL'; 

DELETE FROM BORROW WHERE AMOUNT < 1001;
